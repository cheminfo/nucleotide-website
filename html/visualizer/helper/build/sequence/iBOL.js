...
define(["exports", "https://www.lactame.com/lib/NtSeq/HEAD/NtSeq.js"], function (exports) {
  ...
  var _NtSeq = _interopRequireDefault();
  ...
  function matchDistribution(posSeq, negSeq, useNtSeq) {
    ...
    if (!useNtSeq) {
      ...
      posSeq = posSeq.map(function (seq) {
        return new _NtSeq["default"].Seq().read(seq);
      });
      negSeq = negSeq.map(function (seq) {
        return new _NtSeq["default"].Seq().read(seq);
      });
      primerSet = primerSet.map(function (p) {
        return new _NtSeq["default"].Seq().read(p);
      });
