...
define(["exports", "d3-hierarchy", "src/util/tree", "lodash", "src/main/datas"], function (exports, _d3Hierarchy) {
  ...
  var _tree = _interopRequireDefault();
  ...
  var DataObject = _datas["default"].DataObject; // data should have id as first level property
  ...
  function getTree(data) {
    ...
    tree.each(function (node) {
      node.index = _lodash["default"].property(node, idProperty);
    });
    return tree;
  }
  ...
  function getAnnotatedTree(data) {
    ...
    tree.each(function (node) {
      node.index = idToString(_lodash["default"].get(node.data, idProperty));
      annotations[node.index] = node.data;
    });
    tree = _tree["default"].annotateTree(tree, annotations, annotationOptions);
    return tree;
  }

  function getIdFunction(idProperty) {
    return function getId(d) {
      var id = _lodash["default"].get(d, idProperty);
  ...
  function getCreateParent(idProperty) {
    return function createParent(element) {
      var id = _lodash["default"].get(element, idProperty);

      var parent = {};
      var parentId = id.slice();
      parentId.pop();

      _lodash["default"].set(parent, idProperty, parentId);
