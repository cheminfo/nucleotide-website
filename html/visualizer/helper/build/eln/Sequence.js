...
define(["module", "../biology/translateDNA", "./libs/MolecularFormula"], function (module) {
  var _translateDNA = _interopRequireDefault();
  ...
  function explodeSequences(sample) {
    ...
    if (sequencePeptidic && sequencePeptidic.sequence) {
      sequencePeptidic.sequence = _MolecularFormula["default"].Peptide.sequenceToMF(String(sequencePeptidic.sequence));
    }

    var sequenceNucleic = getFirstNucleotide(sample);

    if (sequenceNucleic && sequenceNucleic.sequence) {
      sequenceNucleic.sequence = _MolecularFormula["default"].Nucleotide.sequenceToMF(String(sequenceNucleic.sequence));
    }
  ...
  function calculateMFFromPeptidic(sample) {
    ...
    if (sequencePeptidic) {
      var sequence = _MolecularFormula["default"].Peptide.sequenceToMF(String(sequencePeptidic.sequence));
  ...
  function calculateMFFromNucleic(sample) {
    ...
    if (sequenceNucleic && sequenceNucleic.sequence) {
      var sequence = _MolecularFormula["default"].Nucleotide.sequenceToMF(sequenceNucleic.sequence, {
          ...
          if (entry.seq) {
            ...
            try {
              for (_iterator2.s(); !(_step2 = _iterator2.n()).done;) {
                ...
                var mf = _MolecularFormula["default"].Nucleotide.sequenceToMF(String(sequence.sequence), {
          ...
          if (_entry.seq) {
            ...
            try {
              for (_iterator4.s(); !(_step4 = _iterator4.n()).done;) {
                ...
                var _mf2 = _MolecularFormula["default"].Peptide.sequenceToMF(String(_sequence.sequence));
                ...
                var _mfInfo2 = new _MolecularFormula["default"].MF(_mf2).getInfo();

                result.peptidic.push({
                  sequence: _sequence,
                  mf: _mf2,
                  mfInfo: _mfInfo2,
                  iep: _MolecularFormula["default"].Peptide.calculateIEP(_mf2)
                });
              }
            } catch (err) {
              _iterator4.e(err);
            ...
            if (antibody.light && antibody.heavy) {
              ...
              var _mfInfo = new _MolecularFormula["default"].MF(_mf).getInfo();
        ...
        if (Array.isArray(nucleic.seq)) {
          ...
          try {
            for (_iterator6.s(); !(_step6 = _iterator6.n()).done;) {
              ...
              peptidic.push({
                sequence: (0, _translateDNA["default"])(entry.sequence)
              });
            }
          } catch (err) {
            _iterator6.e(err);
